...
public:
     ...
     * \brief resetPageNum
     * \param all
     * Resets the page number counter. If all=true then also the global page number will be reset.
     */
    void resetPageNum(bool all = false);
    void resetGlobalPageNum();
    void setGlobalPageNum(int page);
    void nextPageNum();
...
private:
    ...
    */
    ...
    bool updateDataSource( NCReportDataSource* ) const;
    /*! Requeries (sql) datasource that is assigned to a group by group ID */
    bool updateDataSource( const QString& groupID ) const;
